...
func OpenConnection() (db *gorm.DB, err error) {
	...
	case "mysql":
		...
		//case "postgres":
		//	log.Println("db is postgres...")
		//	fmt.Println(fmt.Sprintf("user=%s password=%s dbname=%s host=%s port=%d sslmode=disable TimeZone=%s", Global.Db.Username, Global.Db.Password, Global.Db.Database, Global.Db.Addr, Global.Db.Port, Global.App.TimeZone))
		//	db, err = gorm.Open(postgres.New(postgres.Config{
		//		DSN:                  fmt.Sprintf("user=%s password=%s dbname=%s host=%s port=%d sslmode=disable TimeZone=%s", Global.Db.Username, Global.Db.Password, Global.Db.Database, Global.Db.Addr, Global.Db.Port, Global.App.TimeZone),
		//		PreferSimpleProtocol: true,
		//	}), gormConf)
		//fire.SetFormatColumnType(fire.FormatColumnQuotationMarks)
		//case "sqlserver":
